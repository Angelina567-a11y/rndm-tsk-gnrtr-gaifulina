...
def save_history(history):
    ...
    try:
        ...
        # В случае ошибки просто пропускаем или логируем (если нужно)
...
def main():
    ...
    # Переменные
    global current_task_var, filter_var, new_task_type_var, listbox, new_task_entry
    ...
    # Используем Combobox вместо OptionMenu
    filter_combobox = ttk.Combobox(root, textvariable=filter_var, values=["Все", "учёба", "спорт", "работа"], state="readonly")
    filter_combobox.pack(pady=2)
    filter_combobox.current(0)  # по умолчанию "Все"
    ...
    task_type_combobox = ttk.Combobox(root, textvariable=new_task_type_var, values=["учёба", "спорт", "работа"], state="readonly")
    task_type_combobox.pack(pady=2)
    task_type_combobox.current(0)
